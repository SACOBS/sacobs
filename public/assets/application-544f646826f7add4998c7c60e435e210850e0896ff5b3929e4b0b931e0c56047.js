...
(function() {
  $(document).on("page:change", function() {
    ...
    $('.amount').on('change', function() {
      var total;
      total = 0;
      $('#line_items tr.line_item').each(function() {
        var value;
      ...
      return $('.total').html("<strong> R" + total.toFixed(2) + "</strong>");
    });
    return $('.amount').trigger('change');
  });
  ...
  $(document).on("page:change", function() {
    ...
    $('[data-behaviour~=sortable]').sortable({
      ...
      update: function(e, ui) {
        return $('tr.destination').each(function(index, row) {
          return $(row).find("input[name*='sequence']").val(index + 1);
        });
      }
    });
    return $('.route-destinations').on('cocoon:after-insert', function(event, destination) {
      var next_sequence;
      next_sequence = destination.parent('tbody').prop('rows').length;
      return destination.find("td input[name*='sequence']").val(next_sequence);
    });
  });
...
(function() {
  $(document).on("page:change", function() {
    ...
    $("input[type='radio']").on("change", function() {
      return $connections.detach().filter('[data-route=' + parseInt(this.value) + ']').appendTo('#connections-list');
    });
    $('#connection-search').on('keyup', function() {
      var search_term;
      search_term = this.value.toLowerCase();
      return $connections.detach().filter(function() {
        return $(this).text().toLowerCase().indexOf(search_term) !== -1;
      }).appendTo('#connections-list');
    });
    return $(document).on('ajax:success', '#show_pricing', function(evt, data, status, xhr) {
      return $('.quote').html(data);
    });
  });
...
(function() {
  $(document).on("page:change", function() {
    var calculateCost, calculatePercentage;
    if (!($(".routes").length > 0)) {
      return;
    }
    $('.route-destinations').on('cocoon:after-insert', function(event, destination) {
      destination.find("td input[name*='city']").prop('enabled', true).focus();
      return destination.find("td input[name*='sequence']").val(destination.index() + 1);
    });
    $('.route-destinations').on('cocoon:after-remove', function(event, destination) {
      return $('#destinations').find('tr').each(function(index) {
        return $(this).find("td input[name*='sequence']").val(index);
      });
    });
    $("#connections input[name*='cost']").on('change', function() {
      var $percentageInput, route_cost;
      route_cost = $("input[name~='route[cost]']").val();
      $percentageInput = $(this).closest('tr').find("input[name*='percentage']");
      return $percentageInput.val(calculatePercentage(this.value, route_cost));
    });
    $("#connections input[name*='percentage']").on('change', function() {
      var $costInput, route_cost;
      route_cost = $("input[name~='route[cost]']").val();
      $costInput = $(this).closest('tr').find("input[name*='cost']");
      return $costInput.val(calculateCost(this.value, route_cost));
    });
    $("input[name~='route[cost]']").on('change', function() {
      var $connections, $rows, route_cost;
      route_cost = this.value;
      $connections = $('#connections');
      $rows = $connections.find('tr');
      return $.each($rows, function(index, row) {
        var $costInput, percentage;
        percentage = $(row).find("input[name*='percentage']").val();
        $costInput = $(row).find("input[name*='cost']");
        return $costInput.val(calculateCost(percentage, route_cost));
      });
    });
    calculateCost = function(percentage, amount) {
      return Math.ceil(((percentage / 100 * amount) / 5) * 5);
    };
    return calculatePercentage = function(amount, total) {
...
(function() {
  $(document).on("page:change", function() {
    if (!($(".seasonal-discounts").length > 0)) {
      return;
    }
    return $('#discounted-price, #original-price').on('input', function() {
      var difference_in_price, discounted_price, original_price, percentage;
      ...
      if (!isNaN(discounted_price) || !isNaN(original_price)) {
        ...
        return $('#percentage span').text('%' + percentage.toString());
      }
    });
  });
...
(function() {
  $.rails.allowAction = function(element) {
    ...
    $link = element.clone().removeAttr('class').removeAttr('data-confirm').addClass('btn').addClass('btn-danger').html("Ok");
